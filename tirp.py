...
class TIRP:
    ...
    def __init__(self, stis: list[STI], temp_rels: list[str], vs: float, hs: float):
        """
        ...
        self.vs: float = vs
        self.hs: float = hs
        self.time_point_order: dict = {1: [], 0: [], -1: []}
    ...
    def get_temp_rel_by_ids(self, early_sti_id: int, later_sti_id: int) -> str:
        """
    ...
    def merge_co_occurrence_tieps(self, o_occurrence_tieps, tieps):
        ...
        while i < len(tieps):
            tp: Tiep = tieps[i]
            if any(tp.get_symbol_instance_id() in sublist for sublist in o_occurrence_tieps):
                list_of_values = []
                rel_list = self.in_list(o_occurrence_tieps, tp.get_symbol_instance_id())
    ...
    def update_time_point_comparison(self, first_tiep, second_tiep, comparison_result):
        """
        This function updates the time point comparison dictionary
        :param first_tiep: an endpoint object
        :param second_tiep: an endpoint object
        :param comparison_result: nothing
        :return:
        """
        self.time_point_order[comparison_result].append([first_tiep.get_symbol_instance_id(),
                                                         second_tiep.get_symbol_instance_id()])
    ...
    def sort_tieps_by_relations(self, earlier_tiep: Tiep, later_tiep: Tiep):
        ...
        """
        # earlier tiep values
        earlier_tiep_index = earlier_tiep.get_symbol_instance_id()
        earlier_tiep_type = earlier_tiep.get_tiep_type()

        # later tiep values
        later_tiep_index = later_tiep.get_symbol_instance_id()
        later_tiep_type = later_tiep.get_tiep_type()

        if earlier_tiep_index > later_tiep_index:
        ...
        elif earlier_tiep_index == later_tiep_index:  # if it is the same symbol (for example A and A)
            if earlier_tiep_type == later_tiep_type:
                result = 0  # if it is the same tiep (e.g., A+ and A+)
            elif earlier_tiep_type == const.START_TIEP:
                result = -1  # if the earlier is + and the second is later is - (e.g., A+ and A-)
            else:
                result = 1  # if the earlier is - and the second is later is + (e.g., A- and A+)
        else:
            temp_rel = self.get_temp_rel_by_ids(early_sti_id=earlier_tiep_index,
                                                later_sti_id=later_tiep_index)
            result = self.get_tiep_representation(earlier_tiep_type=earlier_tiep_type,
                                                  temp_rel=temp_rel,
                                                  later_tiep_type=later_tiep_type)
        self.update_time_point_comparison(earlier_tiep, later_tiep, result)
        return result
    ...
    def merge_overlaps_lists(self) -> list:
        ...
        """
        co_occurrence_tieps = self.time_point_order[0].copy()
        is_changed = True
        while is_changed:
